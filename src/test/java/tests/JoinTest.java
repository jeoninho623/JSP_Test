...
public class JoinTest {
    ...
    void joinSuccessByRequest() {
        ...
        given(request.getParameter("confirmUserPw")).willReturn(member.getCheckUserPw());
        given(request.getParameter("userNm")).willReturn(member.getUserNm());
        given(request.getParameter("email")).willReturn(member.getUserEm());
    ...
    void requiredFieldCheck() {
        assertAll(
                ...
                () -> {
                    ...
                    Member member = getMember();
                    member.setCheckUserPw(null);
                    fieldEachCheck(member, "비밀번호를 확인");

                    member.setCheckUserPw("  ");
                    fieldEachCheck(member, "비밀번호를 확인");
                },
                () -> {
                    // 회원명 검증(userNm)
    ...
    @Test
    @DisplayName("아이디(6자리 이상, 첫글자 대문자), 비밀번호(8자리 이상) 최소 자리수 체크, 실패 시 BadRequestException")
    void fieldLengthCheck() {
        assertAll(
                () -> {
                    // 아이디 6자리 이상, 첫글자 대문자 검증
                    Member member = getMember();
                    member.setUserId("user");
                    fieldEachCheck(member,"아이디는 6자리, 첫 글자 대문자");
                },() -> {
                    // 비밀번호 8자리 이상 검증
                    Member member = getMember();
    ...
    void passwordConfirmCheck() {
        BadRequestException thrown = assertThrows(BadRequestException.class, ()-> {
            Member member = getMember();
            member.setCheckUserPw(member.getUserPw() + "**");
            joinService.join(member);
        });
